...
\begin{center}
  \begin{tikzpicture}
    ...
    \tkzDrawPolygon[fill=lightgreen](A,B,C,D)
    ...
    \tkzDefShiftPoint[B](\b,0){B'}
    \tkzDefSquare(A,B') \tkzGetPoints{C'}{D'}
    \tkzDefShiftPoint[C](\b,0){E}
    \tkzDefShiftPoint[C](0,\b){F}

    \tkzDrawPolygon[fill=lightgreen](A,B,C,D)
    \tkzDrawPolygon[fill=lightred](B,B',E,C)
    \tkzDrawPolygon[fill=lightred](D,C,F,D')
    \tkzDrawPolygon[fill=lightorange](C,E,C',F)
...
\[
  (a-b)^{2} = a^{2} - 2ab + b^{2}
\]
Die zweite binomische Formel berechnet die Fläche des Quadrats, welches entsteht, wenn vom Quadrat mit der Seitenlänge $a$ rechts und oben ein Streifen der Breite $b$ abgeschnitten wird. Dabei wird zwei Mal ein Rechteck der Fläche $ab$ abgeschnitten, also $2ab$. Das Quadrat mit der Seitenlänge $b$ wurde dabei jedoch doppelt gezählt. Deshalb muss $b^{2}$ wieder addiert werden.
\begin{center}
  \begin{tikzpicture}
    \def\a{4}
    \def\b{-1}
    ...
    \tkzDefShiftPoint[B](\b,0){B'}
    \tkzDefSquare(A,B') \tkzGetPoints{C'}{D'}
    \tkzDefShiftPoint[C](0,\b){E}
    \tkzDefShiftPoint[C](\b,0){F}

    \tkzDrawPolygon[fill=lightgreen](A,B',C',D')
    \tkzDrawPolygon[fill=orange,opacity=0.3](B',B,C,F)
    \tkzDrawPolygon[fill=orange,opacity=0.3](D',E,C,D)
...
\begin{center}
  \begin{tikzpicture}
    ...
    \tkzDefShiftPoint[A](\a,0){B}
    \tkzDefSquare(A,B) \tkzGetPoints{C}{D}
    \tkzDefShiftPoint[C](-\b,-\b){A'}
    \tkzDefShiftPoint[A'](\b,0){B'}
    \tkzDefSquare(A',B') \tkzGetPoints{C'}{D'}

    \tkzDrawPolygon[fill=lightgreen](A,B,C,D)
    \tkzDrawPolygon[fill=lightorange](A',B',C',D')
    ...
    \tkzLabelSegment[right](B,B'){$a-b$}
    \tkzText(\a-\b/2,\a-\b/2){$b^{2}$}

    \tkzDefPoint(\s,0){A}
    \tkzDefShiftPoint[A](\a+\b,\a-\b){C}
    \tkzDefRectangle(A,C) \tkzGetPoints{B}{D}
    \tkzDefShiftPoint[A](\b,0){E}
    \tkzDefShiftPoint[C](-\b,0){F}
    ...
    \tkzLabelSegment[left](A,D){$a-b$}
    \tkzLabelSegment[below](A,E){$b$}
    \tkzLabelSegment[below](E,B){$a$}
