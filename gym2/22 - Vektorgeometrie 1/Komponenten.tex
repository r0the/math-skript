...
\begin{theorem}
  \textbf{Addition.} Zwei Vektoren $\vv{a} = \tvec{a_{x},a_{y}}$ und $\vv{b} = \tvec{b_{x},b_{y}}$ werden addiert, indem ihre Komponenten addiert werden:
  \[
    \vv{a}+\vv{b} = \dvec{a_{x},a_{y}}+\dvec{b_{x},b_{y}} = \dvec{a_{a}+b_{x},a_{y}+b_{y}}
  \]
...
\begin{theorem}
  \textbf{Skalierung.} Ein Vektor  $\vv{a} = \tvec{a_{x},a_{y}}$ wird skaliert, indem seine Komponenten mit dem Faktor multipliziert werden:
  \[
    k\cdot\vv{a} = k\cdot\dvec{a_{x},a_{y}} = \dvec{k\cdot a_{x},k\cdot a_{y}}
  \]
\end{theorem}
...
\begin{theorem}
  \textbf{Länge eines Vektors.} Die Länge $|\vv{a}|$ eines Vektors $\vv{a} = \tvec{a_{x},a_{y}}$ wird wie folgt berechnet:
  \[
...
\begin{example}
  \textbf{Beispiel:} In der oben stehenden Abbildung hat der Vektor $\vv{a}$ die Komponenten $\tvec{5,3}$. Also ist seine Länge
  \[
...
\[
  \dvec{a_{x},a_{y}} = \dvec{b_{x},b_{y}} \qquad\Leftrightarrow\qquad a_{x} = b_{x} \quad\text{und}\quad a_{y} = b_{y}
\]