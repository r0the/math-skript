...
\begin{example}
  ...
  \[
    g:\quad \vv{r} = \tvec{4,3}+t\cdot \tvec{2,-0.5}
  \]
  ...
  \begin{align*}
    \vv{r_{1}} &= \tvec{4,3}+2\cdot \tvec{2,-0.5} = \tvec{4+4,3-1} = \tvec{8,2} \qquad&&\Rightarrow\qquad \coord[R_{1}]{8,2} \\
    \vv{r_{2}} &= \tvec{4,3}-1.5\cdot \tvec{2,-0.5} = \tvec{4-3,3+0.75} = \tvec{1,3.75} \qquad&&\Rightarrow\qquad \coord[R_{2}]{1,3.75}
  \end{align*}
\end{example}
...
\]
\begin{example}
  \textbf{Beispiel:} Die Gleichung der Geraden durch $\coord[A]{2,3}$ und
  $\coord[B]{-4,2}$ soll bestimmt werden.
  \begin{align*}
    \vv{a} &= \vv{AB} = \vv{OB}-\vv{OA} = \tvec{-4,2}-\tvec{2,3} = \tvec{-6,-1} \\
    \vv{p} &= \vv{OA} = \tvec{2,3} \\
    \vv{r} &= \vv{p}+t\cdot \vv{a} = \tvec{2,3}+t\cdot \tvec{-6,-1}
  \end{align*}
\end{example}
% ------------------------------------------------------------------------------
