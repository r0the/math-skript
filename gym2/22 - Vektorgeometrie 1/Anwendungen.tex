...
\[
  \coord[A]{2,4} \qquad \coord[B]{11,1}
\]
...
\begin{center}
  \begin{tikzpicture}
    ...
    \tkzDrawSegment[dashed,vector style](O,T1)
    \tkzDrawSegment[dashed,vector style](O,T2)
...
\begin{align*}
       \vv{a} &= \vv{AB} = \tvec{11,1} - \tvec{2,4} = \tvec{11-2,1-4} = \tvec{9,-3} \\[2mm]
  \vv{OT_{1}} &= \tvec{2,4}+\frac{1}{3}\tvec{9,-3} = \tvec{2+\frac{1}{3}9,4+\frac{1}{3}(-3)} = \tvec{5,3} \\[2mm]
  \vv{OT_{2}} &= \tvec{2,4}+\frac{2}{3}\tvec{9,-3} = \tvec{2+\frac{2}{3}9,4+\frac{2}{3}(-3)} = \tvec{8,2}
\end{align*}
Die Strecke wird also durch die Punkte $\coord[T_{1}]{5,3}$ und
$\coord[T_{2}]{8,2}$ in drei gleiche Teile geteilt.

% ------------------------------------------------------------------------------
\subsection{Umfang eines Dreiecks}
\textbf{Aufgabe:} Bestimmen Sie den Umfang des Dreiecks $\triangle ABC$ mit
\[
  \coord[A]{3,2} \qquad \coord[B]{8,1} \qquad \coord[C]{7,4}
\]
...
\begin{align*}
  \vv{a} &= \vv{AB} = \tvec{8-3,1-2} = \tvec{5,-1} & |\vv{a}| &= \sqrt{5^{2}+(-1)^{2}} = \sqrt{26} \approx 5.099 \\[2mm]
  \vv{b} &= \vv{BC} = \tvec{7-8,4-1} = \tvec{-1,3} & |\vv{b}| &= \sqrt{(-1)^{2}+3^{2}} = \sqrt{10} \approx 3.162 \\[2mm]
  \vv{c} &= \vv{CA} = \tvec{3-7,2-4} = \tvec{-4,-2} & |\vv{c}| &= \sqrt{(-4)^{2}+(-2)^{2}} = \sqrt{20} \approx 4.472
\end{align*}
...
\[
  \coord[A]{2,2} \qquad \coord[B]{9,1} \qquad \coord[C]{7,4}
\]
\begin{center}
  \begin{tikzpicture}
    \tkzInit[xmin=0,xmax=12,ymin=0,ymax=5]
...
\[
  \vv{a} = \vv{BC} = \tvec{7-9,4-1} = \tvec{-2,3}
\]
Nun kann der Mittelpunkt der Seite $a$ berechnet werden:
\[
  \vv{OM} = \vv{OB}+\tfrac{1}{2}\vv{a} = \tvec{9,1}+\tfrac{1}{2}\tvec{-2,3} = \tvec{8,2.5}
\]
Jetzt wird der Vektor $\vv{AM}$ berechnet, welcher die Seitenhalbierende beschreibt.
\[
  \vv{AM} = \vv{OM}-\vv{OA} = \tvec{8,2.5}-\tvec{2,2} = \tvec{6,0.5}
\]
Da der Schwerpunkt die Seitenhalbierende im Verhältnis 2:1 teilt, kann nun dessen Ortsverktor berechnet werden.
\[
  \vv{OS} = \vv{OA}+\tfrac{2}{3}\vv{AM} = \tvec{2,2}+\tfrac{2}{3}\tvec{6,0.5} = \tvec{6,2.\overline{3}}
\]
Der Schwerpunkt des Dreiecks hat also die Koordinaten
$\coord[S]{6,2.\overline{3}}$.
\newpage
% ------------------------------------------------------------------------------
...
\[
  \coord[A]{3,2} \qquad \coord[B]{8,1} \qquad \coord[D]{6,4}
\]
Zuerst werden die Seitenvektoren des Parallelogramms bestimmt:
\begin{align*}
  \vv{a} &= \vv{OB}-\vv{OA} = \tvec{8,1}-\tvec{3,2} = \tvec{8-3,1-2} = \tvec{5,-1} \\[2mm]
  \vv{b} &= \vv{OD}-\vv{OA} = \tvec{6,4}-\tvec{3,2} = \tvec{6-3,4-2} = \tvec{3,2}
\end{align*}
Der Punkt $C$ erhält man, indem der Punkt $B$ um den Vektor $\vv{b}$ verschiebt. Dazu wird $\vv{b}$ zum Ortsvektor $\vv{OB}$ addiert:
\[
  \vv{OC} = \vv{OB} + \vv{b} = \tvec{8,1} + \tvec{3,2} = \tvec{8+3,1+2} = \tvec{11,3}
\]
Also ist der vierte Punkt des Parallelogramms $\coord[C]{11,3}$.
\begin{center}
  \begin{tikzpicture}
    \tkzInit[xmin=0,xmax=12,ymin=0,ymax=5]
    \tkzAxeXY[]
...
\[
  \vv{a} = \tvec{6,1}-\tvec{3,2} = \tvec{6-3,1-2} = \tvec{3,-1}
\]
...
\begin{align*}
  \vv{OC} &= \vv{OB}+\vv{b} = \tvec{6,1} + \tvec{1,3} = \tvec{6+1,1+3} = \tvec{7,4} \\[2mm]
  \vv{OD} &= \vv{OA}+\vv{b} = \tvec{3,2} + \tvec{1,3} = \tvec{3+1,2+3} = \tvec{4,5}
\end{align*}
Die gesuchten Punkte sind also $\coord[C]{7,4}$ und $\coord[D]{4,5}$. Die Fläche
...
\[
  \coord[A]{3,2} \qquad \coord[B]{6,5} \qquad \coord[C]{7,2}
\]
\begin{center}
  \begin{tikzpicture}
...
\[
  \vv{OA} = \tvec{3,2} \qquad \vv{OM} = \tvec{x,y}
\]
...
\begin{align*}
  ...
  8x &= 40
\end{align*}
Die Lösung des Gleichungssystems ist $x = 5$ und $y = 3$. Also hat der Mittelpunkt des Kreises die Koordinaten $\coord[M]{5,3}$.

Den Radius des Kreises ist die Länge des Vektors $\vv{AM}$:
\[
  r = \left|\vv{AM}\right| = \left|\tvec{5-2,3-2}\right| = \left|\tvec{3,1}\right| = \sqrt{3^{2}+1^{2}} = \sqrt{10} \approx 3.162
\]
