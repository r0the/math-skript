...
\begin{theorem}
  \textbf{Kollinearität.} Zwei Vektoren $\vv{a} = \tvec{a_{x},a_{y}}$ und $\vv{b} = \tvec{b_{x},b_{y}}$ sind kollinear (parallel), wenn es eine Zahl $t$ gibt, sodass
  \[
    \vv{a} = t\cdot\vv{b} \qquad\Leftrightarrow\qquad \dvec{a_{x},a_{y}} = t\cdot \dvec{b_{x},b_{y}}
  \]
\end{theorem}
...
\begin{example}
  \textbf{Beispiele:} Die Vektoren $\vv{a} = \tvec{2,-1}$ und $\vv{b} = \tvec{-6,3}$ sind kollinear, da:
  \[
     2 = \mathcolor{red}{-\frac{1}{3}\cdot} (-6) \qquad\qquad -1 = \mathcolor{red}{-\frac{1}{3}\cdot} 3
  \]

  Die Vektoren $\vv{a} = \tvec{2,-1}$ und $\vv{c} = \tvec{2,2}$ sind nicht parallel, da
  \[
...
\begin{theorem}
  \textbf{Orthogonalität.} Zwei Vektoren $\vv{a} = \tvec{a_{x},a_{y}}$ und $\vv{b} = \tvec{b_{x},b_{y}}$ sind orthogonal (stehen senkrecht zueinander), wenn
  \[
...
\begin{example}
  \textbf{Beispiele:} Die Vektoren $\vv{a} = \tvec{2,-3}$ und $\vv{b} = \tvec{6,4}$ sind orthogonal, da:
  \[
    2\cdot 6 + (-3)\cdot 4 = 12 - 12 = 0
  \]
  Die Vektoren $\vv{a} = \tvec{2,-3}$ und $\vv{c} = \tvec{1,4}$ sind nicht orthogonal, da
  \[
...
\begin{theorem}
  \textbf{Senkrechter Vektor.} Ist der Vektor $\vv{a} = \tvec{a_{x},a_{y}}$ gegeben, so stehen die Vektoren
  \[
    \dvec{-a_{y},a_{x}} \qquad\qquad \dvec{a_{y},-a_{x}}
  \]
...
\begin{example}
  \textbf{Beispiel:} Zum $\vv{a} = \tvec{2,-3}$ stehen die folgenden beiden Vektoren senkrecht:
  \[
    \vv{b} = \dvec{3,2} \qquad\qquad -\vv{b} = \dvec{-3,-2}
  \]
\end{example}
